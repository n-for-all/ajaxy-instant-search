(function ($) {
	...
	$.ajaxyLiveSearch = {
		...
		load: function (elem, options, arg) {
			...
			$(elem).attr("placeholder", options.text);
			$(elem).attr("autocomplete", "off");
			if ($("#sf_sb").length == 0) {
				$("body").append(
					'<div id="sf_sb" class="sf_sb" style="position:absolute;display:none;width:' +
						options.width +
						';z-index:9999">' +
		...
		loadResults: function (object) {
			...
			if (jQuery(elem).val() != "") {
				...
				var loading = '<li class="sf_lnk sf_more sf_selected">' + '<a id="sf_loading" href="' + options.searchUrl.replace("%s", encodeURI(jQuery(elem).val())) + '">' + "</a>" + "</li>";
		...
		loadEvents: function (object) {
			...
			jQuery(object.element).keyup(function (event) {
				if (event.keyCode != "38" && event.keyCode != "40" && event.keyCode != "13" && event.keyCode != "27" && event.keyCode != "39" && event.keyCode != "37") {
					...
					var ajaxyObjectClass = jQuery(ajaxyObject.element).attr("class");
					if (!ajaxyObjectClass) {
						ajaxyObjectClass = "";
					}
					jQuery(ajaxyObject.element).attr("class", ajaxyObjectClass.replace(" sf_focused", "") + " sf_focused");
			...
			jQuery(object.element).blur(function () {
				this.classList.remove("sf_focused");
				if (d.object.options.expand > 0) { 
					jQuery(d.object.element).animate({
